...
def make_meta_label(df):
    ...
    for i, v in tqdm(df.iterrows()):
        if v["scratchpos_baffle"]!=0:
            label = 1
            baf += 1
        elif v["scratchpos_center"]!=0:
            label = 2
            cen +=1
        elif v["scratchpos_edge"]!=0:
            label = 3
            edge += 1
        else:
            label = 0
        ...
        if i < len(df)-200:
            ...
            val.append(label)
    print("normal is {}, each anomaly of baffle, center, edge are {} {} {}".format(len(df)-c, baf, cen, edge))
