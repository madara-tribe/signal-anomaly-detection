...
class Trainer:
    ...
    def call_data_loader(self, config, num_worker):
        ...
        self.train_dst = MetaDataLoader(config, config.x1, config.x2, config.y, transform=train_transform, valid=None)
        self.val_dst = MetaDataLoader(config, config.x1, config.x2, config.y, transform=val_transform, valid=True)
